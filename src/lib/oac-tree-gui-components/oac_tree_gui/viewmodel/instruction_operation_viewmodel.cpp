...
private:
  std::vector<std::unique_ptr<mvvm::ViewItem>> ConstructRowImpl(mvvm::SessionItem* item) override
  {
    ...
    {
      // InstructionItem has editable display name, here we want it read-only, so using label view
      // item instead of CreateDisplayViewItem.
      result.emplace_back(mvvm::CreateLabelViewItem(instruction, instruction->GetDisplayName()));
