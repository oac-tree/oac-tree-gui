    ...
    const UserContext& user_context, IRemoteConnectionService& service)
{
                 ...
                 &service](JobItem& item) -> std::unique_ptr<AbstractJobHandler>
  {
    ...
    {
      return std::make_unique<LocalJobHandler>(&item, std::move(user_context));
    }

    if (item.GetType() == RemoteJobItem::Type)
    {
      // remote jobs need special RemoteJobHandler generated by the remote connection service
      return service.CreateJobHandler(dynamic_cast<RemoteJobItem*>(&item), std::move(user_context));
    }
