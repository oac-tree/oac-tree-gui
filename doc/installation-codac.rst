...
Run one of related applications from the command line::
...
CODAC Operation Applications repositories are located at `COA repositories <https://git.iter.org/scm/coa>`_.
Checkout all packages from the list, compile and install them one-by-one, using `mvn clean install` command::
