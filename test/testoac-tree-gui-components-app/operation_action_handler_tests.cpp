...
TEST_F(OperationActionHandlerTest, SubmitLocalJob)
{
  ...
  EXPECT_EQ(mvvm::test::TakeValue<const JobItem*>(spy_selected_request), job_items.at(0));
}
...
TEST_F(OperationActionHandlerTest, SubmitThrowingLocalJob)
{
  ...
  EXPECT_EQ(mvvm::test::TakeValue<const JobItem*>(spy_selected_request), job);
}
...
TEST_F(OperationActionHandlerTest, OnImportRemoteJobRequest)
{
  ...
  const RemoteConnectionInfo connection_context{EPICSServerInfo{server_name}, {job_index}};
...
TEST_F(OperationActionHandlerTest, ImportTwoRemoteJobs)
{
  ...
  const RemoteConnectionInfo connection_context{EPICSServerInfo{server_name},
                                                {job_index0, job_index1}};
...
TEST_F(OperationActionHandlerTest, RemoveLocalJobInTheMiddle)
{
  ...
  EXPECT_EQ(mvvm::test::TakeValue<const JobItem*>(spy_selected_request), job_item2);
}
